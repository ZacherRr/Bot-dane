"""Бот Вкнотакте для танцевальной студии
Разработчик Хоран Евгений"""
...
class Bot:
    """Класс содержит в себе все функции для взаимодействия с ботом"""
    ...
    def session(tok):
        ...
        sessions = vk_api.VkApi(token=tok)
        return sessions
    ...
    def send(self):
        ...
        vk.messages.send(peer_id=self.id, random_id=self.random_id,
                         message=self.text, keyboard=self.keyboard3, attachment=attachment)
    ...
    def notice(id_client, flag):
        ...
        if flag == "{\"button\":\"time\"}":
            vk.messages.send(peer_ids=ids_admins, random_id=message_id,
                             message=f'Пользователь записался на занятие\n'+text)
        # Если текст кнопки был 'Позвать человека'
        elif flag == 'Позвать человека':
            vk.messages.send(peer_ids=ids_admins, random_id=message_id,
                             message=f'Пользователь просит помощи у человека\n'+text)
...
class Keyboard:
    """Класс содержит все функции для взаимодействия с клавиатурой"""
...
class Data:
    ...
    @staticmethod
    def add_id_user(id_client, vk_api, status):
        """Метод для добавления айди, ФИ"""
        # Использвуем метод из vk api, которого нет в бибиотеке vk_api
        user = vk_api.method("users.get", {"user_ids": id_client})
        columns = ['id_user', 'first_name', 'last_name', 'group', 'time', 'phone', 'status', 'datetime']
        df = pd.DataFrame([[str(id_client), user[0]['first_name'],
                            user[0]['last_name'], '', '', '', status, '']], columns=columns)
    ...
    @staticmethod
    def add_time(id_client, times):
        """Метод для добавления расписания группы"""
        df = pd.read_csv('/home/Zacher/mysite/forms.csv')
        df.loc[df.id_user == id_client, 'time'] = times
        df.to_csv('/home/Zacher/mysite/forms.csv', index=False)
    ...
    def add_status(id_client, status):
        """Метод для добавления номера статуса"""
        df = pd.read_csv('/home/Zacher/mysite/forms.csv')
    ...
    @staticmethod
    def logs(id_client, message, times):
        """Метод для ведения лога"""
        with open('/home/Zacher/mysite/logs.csv', 'a') as file:
            file.write(f'{id_client},{message},{int(times)}\n')


class Commands:
    """Класс содержит в себе все функции для обработки команд администратора"""
    ...
    def add_admin(id_user, level):
        ...
        with open(admin_list, encoding='utf-8') as file:
            admins = json.load(file)
        if not admins['administrators'].get(id_user, False):
    ...
    def remove_admin(id_admin):
        """Метод для удаления администраторов"""
    ...
    @staticmethod
    def check_admin():
        """Метод для проверки наличия административного доступа у пользователя"""
        check_list = '/home/Zacher/mysite/commands_data.json'
        with open(check_list, encoding='utf-8') as file:
            check_json = json.load(file)
    ...
    def access_spam(id_admin, random_id, message_id):
        ...
        if Commands.lock_command('check', message_id):
            ...
            finish = time.time()
            Commands.lock_command('unlock', message_id)
            Bot.admin_send(id_admin, random_id, f'Список для рассылки обновлен за '
                                                f'{int(finish-start)} секунд. В списке {count_spam_list}')
        else:
...
def message_new(text_button, text_chat, ids, random_id, payload1, attachment, one_time, inline):
    ...
    bot_sender = Bot(ids, Bot.cliche_text[text_chat], random_id, keyboard, attachment)
    bot_sender.send()
...
def events():
    ...
    # если же это сообщение, отвечаем пользователю
        ...
        text_user = Smiler(data['object']['message']['text'])
        ...
        # Если сообщение это не команда и пишет пользователь не нажавший кнопку Я ученик,
        # либо фразу Начать (чтобы любой мог вернуть бота)
        ...
        if (check_status or text_user == 'Начать') and text_user[0] != '/':
            ...
            elif None is not re.search(r'^((8|\+7)[\- ]?)?(\(?\d{3}\)?[\- ]?)?[\d\- ]{7,10}$', text_user):
            ...
            elif None is data['object']['message'].get('payload'):
                if Data.check(id_user):
                    message_new('Начать', 'Начать', id_user, message_id,
                                "{\"button\":\"start\"}", not_img, False, True)
                else:
                    message_new('Непонятно', 'Непонятно', id_user, message_id,
                                "{\"button\":\"start\"}", not_img, False, True)
            ...
            # Отправляем сообщение по нажатию кнопки Назад
                ...
                # Если кнопка была нажата в разделе расписания
                    ...
                    message_new('Записаться', 'Записаться_назад', id_user, message_id,
                                "{\"button\":\"group\"}", not_img, True, False)
            ...
            # Выбор сообщения после нажатия Начать
                ...
                if text_user == 'Записаться':
                    ...
                    if Data.check(id_user):
                        message_new(text_user, text_user, id_user, message_id,
                                    "{\"button\":\"group\"}", img_no_button, True, False)
                        Data.add_id_user(int(id_user), vk_session, 'Новый')
                    else:
                        message_new(text_user, text_user+'_назад', id_user, message_id,
                                    "{\"button\":\"group\"}", not_img, True, False)
                ...
                # Отправляем сообщение по нажатию кнопки Задать вопрос
                    ...
                    if Data.check(id_user):
                        ...
                        message_new(text_user, text_user, id_user, message_id,
                                    "{\"button\":\"question\"}", img_no_button, True, False)
                        Data.add_id_user(int(id_user), vk_session, 'Новый')
                    else:
                        message_new(text_user, text_user+'_назад', id_user, message_id,
                                    "{\"button\":\"question\"}", not_img, True, False)
            ...
            # Отправка ответа на вопросы
                ...
                if text_user == 'Расписание':
                    ...
                    message_new('Задать вопрос', text_user, id_user, message_id,
                                "{\"button\":\"question\"}", timesheet, True, False)
            ...
            # Отправляем сообщение по нажатию кнопки Группы
                ...
                message_new(text_user, "Выбрать время", id_user, message_id,
                            "{\"button\":\"time\"}", not_img, True, False)
            ...
            if level_admin in [1, 2, 3] and id_message != id_message_commands:
                if check_commands:
                    if parrametrs[0] == '/help':
                        message_new('', "Liza_code", id_user, message_id, "empty",
                                    'audio2000141460_456240403', True, False)
